...
impl MessagePayload for AnnounceShardRequest {
    ...
    fn serialize(&self) -> Result<Vec<u8>> {
        ...
        buffer.push(self.shard_type as u8);
    ...
    fn deserialize(buffer: &[u8]) -> Result<Self> {
        ...
        let shard_type: ShardType = ShardType::try_from(buffer[offset]).unwrap();
        offset += 1;

        // Read IP (16 bytes)
        let ip =
            u128::from_le_bytes(buffer.get(offset..offset + 16).context("failed to get IP bytes")?.try_into()?);
        offset += 16;
